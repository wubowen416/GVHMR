...
class CorrTransformer:
    def __init__(self):
        self.R_a2w = R.from_euler("ZXY", np.array([0, -3.08, -180.0]), degrees=True).as_matrix()
        self.T_a2w = np.array([0, 0, 0])
        self.R_b2w = R.from_euler(
            "ZXY", np.array([-0.597, 1.53, -146.691]), degrees=True
        ).as_matrix()  # X changed from -4.53 to 1.52 to compensate
        self.T_b2w = np.array([0, 0, 0])

    def transform(self, X, P):
        X_out = self.R_a2w.T @ self.R_b2w @ X + self.R_a2w.T @ (self.T_b2w - self.T_a2w)
        P_out = self.R_a2w.T @ self.R_b2w @ R.from_rotvec(P).as_matrix()
        return X_out, R.from_matrix(P_out).as_rotvec()
...
if __name__ == "__main__":
    ...
    if True:
        ...
        # Output json
        corr_transformer = CorrTransformer()
        smpl_params = pred["smpl_params_global"]
        out_dict = {"frames": [], "betas": smpl_params["betas"][0].numpy().tolist()}
        T = smpl_params["transl"].shape[0]
        frame_times = np.linspace(0, T / 30, T)
        prev_ct_transl = np.zeros(3)
        for t in range(T):
            ct_transl, ct_orient = corr_transformer.transform(
                smpl_params["transl"][t].numpy(), smpl_params["global_orient"][t].numpy()
            )
            if t == 0:
                transl_rel = np.zeros(3)
                ct_transl_rel = np.zeros(3)
            else:
                transl_rel = (smpl_params["transl"][t] - smpl_params["transl"][t - 1]).numpy()
                ct_transl_rel = ct_transl - prev_ct_transl
            prev_ct_transl = ct_transl

            # Save frame
            frame_dict = {
                "body_pose": smpl_params["body_pose"][t].numpy().tolist(),
                "global_orient": smpl_params["global_orient"][t].numpy().tolist(),
                "transl": smpl_params["transl"][t].numpy().tolist(),
                "ct_transl": ct_transl.tolist(),
                "ct_transl_rel": ct_transl_rel.tolist(),
                "ct_orient": ct_orient.tolist(),
                "transl_rel": transl_rel.tolist(),
                "frame_time": float(frame_times[t]),
            }
            out_dict["frames"].append(frame_dict)
        with open(paths.hmr4d_results.replace(".pt", ".json"), "w") as f:
            import json

            json.dump(out_dict, f, indent=2)

        # Output .npz file for smplx
        poses = np.zeros((T, 55, 3), dtype=np.float32)
        poses[:, 0] = smpl_params["global_orient"].numpy()
        poses[:, 1:22] = smpl_params["body_pose"].numpy().reshape(T, -1, 3)
        smplx_dict = {
        ...
        np.savez(paths.hmr4d_results.replace(".pt", ".npz"), **smplx_dict)
